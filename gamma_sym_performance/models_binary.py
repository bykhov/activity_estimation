...
def unet_model4(frame_length, n_filters=16):
    ...
    outputs = Dense(1, activation='sigmoid')(conv10)

    model = tf.keras.Model(inputs=inputs, outputs=outputs)
    print('U-Net 4 Model, filters: ', n_filters, ', parameters: ', model.count_params(), )
    return model
...
def unet_model3(frame_length, n_filters=16):
    ...
    model = tf.keras.Model(inputs=inputs, outputs=outputs)
    print('U-Net 3 Model, filters: ', n_filters, ', parameters: ', model.count_params(), )
    return model
...
def unet_model2(frame_length, n_filters=16):
    ...
    model = tf.keras.Model(inputs=inputs, outputs=outputs)
    print('U-Net 2 Model, filters: ', n_filters, ', parameters: ', model.count_params(), )
    return model
...
def lstm_model(lstm_depth=1, inception_depth=2, filters=8, lstm_length=32, sample_length=1024):
    ...
    model = Model(inputs=signal_input, outputs=x)
    print('LSTM Model, filters: ', filters, ', lstm_length: ', lstm_length, ', lstm_depth: ', lstm_depth,
          ', parameters: ', model.count_params())
    return model


# %% CNN Model
def cnn_model(conv2d_depth=1, inception_depth=3, filters=16, sample_length=1024):
    """Create model"""
    signal_input = Input(shape=[sample_length, 1], name='samples')
    x = signal_input
    for _ in range(inception_depth):
    ...
    for _ in range(conv2d_depth):
        x31 = Conv2D(filters=16, kernel_size=5, padding='same')(x)
        x32 = Conv2D(filters=16, kernel_size=3, padding='same')(x)
        x33 = Conv2D(filters=16, kernel_size=1, padding='same')(x)
        x = Concatenate()([x31, x32, x33])
        x = BatchNormalization()(x)
        x = Activation('relu')(x)
        x = Dropout(0.1)(x)

    x = Reshape((sample_length, -1))(x)
    x = Dense(1, activation='sigmoid')(x)
    model = Model(inputs=signal_input, outputs=x)
    print('CNN Model, filters: ', filters, ', conv2d_depth: ', conv2d_depth, ', parameters: ', model.count_params())
    return model
